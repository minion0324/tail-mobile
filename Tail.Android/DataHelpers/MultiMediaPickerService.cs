...

{
    ...
    {
        public static MultiMediaPickerService SharedInstance { get; set; } = new MultiMediaPickerService();
        readonly int MultiPickerResultCode = 9793;
        ...
        public void OnActivityResult(int requestCode, Result resultCode, Intent data)
        {
            ...
            if (requestCode == MultiPickerResultCode)
            {
                ...
                if (resultCode == Result.Ok && data != null)
                {
                    ...
                    AddClipData(clipData, mediaPicked, data); 
                }
        ...
        private void AddClipData(ClipData clipData, ObservableCollection<MediaFile> mediaPicked, Intent data)
        {
            if (clipData != null)
            {
                AddMedia(clipData, mediaPicked);
            }
            else
            {
                Android.Net.Uri uri = data.Data;
                AddMediaUri(uri, mediaPicked);
            }
            OnMediaPickedCompleted?.Invoke(this, mediaPicked);
        }
        private void AddMedia(ClipData clipData, ObservableCollection<MediaFile> mediaPicked)
        {
        ...
        private void AddMediaUri(Android.Net.Uri uri, ObservableCollection<MediaFile> mediaPicked)
        {
            var media = CreateMediaFileFromUri(uri);
            if (media != null)
            {
